...
# torbrowser, firefox, thunderbird, seamonkey, firefox-bin, thunderbird-bin, seamonkey-bin
#
...
mozilla-addon_verifytarget() {
	...
	case ${1} in
		firefox|torbrowser|firefox-bin)
...
mozilla-addon_src_install() {
	...
	if ! has 'firefox' $MZA_TARGETS \
		...
		die "At least one of these targets (firefox torbrowser thunderbird seamonkey firefox-bin thunderbird-bin seamonkey-bin) must be set to use this eclass."
	fi
	...
	# target: torbrowser
	if has 'torbrowser' $MZA_TARGETS; then
		mozilla-addon_verifytarget torbrowser
		mozilla-addon_dosym "${installpath}" \
			"/usr/$(get_libdir)/torbrowser/browser/extensions/${emid}"
		count=$((count+1))
		mozilla-addon_dosym "${installpath}" \
			"/usr/$(get_libdir)/torbrowser/extensions/${emid}"
		count=$((count+1))
	fi
	# target: thunderbird
