...
src_configure() {
	...
	econf \
		...
		--mandir="${EPREFIX}"/usr/share/man/man1/guile-${MAJOR} \
...
src_install() {
	#emake DESTDIR="${D}" infodir="${ED}"/usr/share/info/guile-${MAJOR} install
	#emake infodir="${ED}"/usr/share/info/guile-${MAJOR} install
